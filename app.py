...
DIGER_ALANLAR = {
    "Evrak Kayıt / Vezne": {"fancy": False, "tarif": "Zemin Kat - Ana girişten tam karşınızda.", "kat": "zemin", "kroki": "krokiler/kroki_saglik_kurulu_kayit.png"},
    "Hasta Kayıt": {"fancy": False, "tarif": "Ana girişte sol tarafta yer alır.", "kat": "zemin", "kroki": "krokiler/kroki_saglik_kurulu_kayit.png"},
    "Evde Sağlık Hizmetleri Birimi": {"fancy": False, "tarif": "Zemin Katta olup girişi binanın kuzey yönündedir.", "kat": "zemin", "kroki": ""},
    "Röntgen / Görüntüleme (DİĞER BİNA)": {"fancy": True, "tarif": "Diğer binadadır! Röntgen birimi bu binada değildir. Arka kapıdan çıkınca sola dönün, ardından sağa, ileride sağ tarafta yer alır.", "kat": "", "kroki": ""},
    "Asansör": {"fancy": False, "tarif": "1. katta binanın tam orta kesiminde, zemin katta Hasta Kayıt bankosunun geçince sol tarafta yer alır.", "kat": "zemin", "kroki": ""},
    "Tuvaletler / Lavabolar": {"fancy": False, "tarif": "Zemin Katta: Ana girişten sonra sola dönün. Koridorun sonunda yer alır.", "kat": "zemin", "kroki": ""}
}

ES_ANLAMLILAR = {
    "kan": "Kan Alma Birimi", "tahlil": "Kan Alma Birimi", "laboratuvar": "Kan Alma Birimi", "tahlili": "Kan Alma Birimi",
    ...
    "göğüs": "Poliklinik Heyet Göğüs Hastalıkları",
    "üroloji": "Poliklinik Heyet Üroloji", "cerrahi": "Poliklinik Heyet Genel Cerrahi",
    "röntgen": "Röntgen / Görüntüleme (DİĞER BİNA)", "film": "Röntgen / Görüntüleme (DİĞER BİNA)",
    "işitme": "İşitme Testi (ODİO)", "odio": "İşitme Testi (ODİO)",
    "dahiliye": "Poliklinik Heyet Dahiliye", "nöroloji": "Poliklinik Heyet Nöroloji",
    "ortopedi": "Poliklinik Heyet Ortopedi", "psikiyatri": "Poliklinik Psikiyatri",
    "cimer": "Sabim Cimer Birimi", "sft": "Solunum Fonksiyon (SFT) Birimi"
}
...
def kroki_goster(dosya_yolu):
    """
    POLIKLINIKLER sözlüğünde tanımlı dosya yolunu (.png / .jpg uzantısı veya büyük-küçük harf bağımsız) 
    otomatik olarak bulur ve ekranda gösterir.
    """
    if not dosya_yolu:
        return
    ...
    # 1. Tam dosya yolu kontrolü
    if os.path.exists(dosya_yolu):
        bulunan_dosya = dosya_yolu
    else:
        # 2. Uzantı veya harf duyarlılığını esneterek arama
    ...
    if bulunan_dosya:
        try:
            ...
            st.image(image, caption="🗺️ Yol Tarifi Krokisi", use_container_width=True)
        except Exception as e:
            st.error(f"🚨 Kroki görseli yüklenirken hata oluştu: {e}")
    else:
        st.warning(f"📸 Kroki dosyası bulunamadı: `{dosya_yolu}`. Lütfen dosyayı GitHub'daki `krokiler/` klasörüne yüklediğinizden emin olun.")
...
if secim != "Seçim Yapınız...":
    ...
    if secim in tum_birimler:
        ...
        if veri['fancy']:
            ...
            # İlgili polikliniğe ait kroki görselini yükle
            if veri.get('kroki'):
                kroki_goster(veri['kroki'])
